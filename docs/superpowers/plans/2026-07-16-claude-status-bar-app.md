...
- Shared test mocks (`MockHTTPClient`, `ManualClock`) live in a **non-test `TestSupport` library target** (SwiftPM forbids a test target depending on another test target). Both `ClaudeStatusBarCoreTests` and `ClaudeStatusBarAppTests` depend on `TestSupport` and `import TestSupport` where they use those mocks.
